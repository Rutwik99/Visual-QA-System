...
# Opening JSON file
f = open('data/questions/v2_OpenEnded_mscoco_train2014_questions_yes_no.json')
...
# Read annotations
f = open('data/annotations/v2_mscoco_train2014_annotations_yes_no.json')
...
class VQADataset(torch.utils.data.Dataset):
	"""Dataset class for VQA (v2) dataset."""
...
if i >= 0:
	for name, param in model.named_parameters():
		...
		elif name.startswith('model.encoder.layer.' + str(i)):
			break
		elif not name.startswith('model.embeddings') and not name.startswith('model.encoder'):
...
val_dataloader = DataLoader(dataset_val, collate_fn=collate_fn, batch_size=32, shuffle=False, num_workers=6)
